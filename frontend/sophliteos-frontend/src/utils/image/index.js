...
export function drawRectOnImage(imageUrl, srcRect, InBoxs) {
  return new Promise((resolve, reject) => {
    const img = new Image();
    const hasManyBox = Array.isArray(srcRect);
    img.setAttribute('crossOrigin', 'anonymous');
    img.onload = () => {
      const canvas = document.createElement('canvas');
      ...
      if (hasManyBox) {
        srcRect.map((item, index) => {
          ...
          InBoxs && ctx.fillText(InBoxs[index].confidence.toFixed(4), item.x, item.y - 5);
        });
      } else {
        ctx.rect(srcRect.x, srcRect.y, srcRect.width, srcRect.height);
        InBoxs && ctx.fillText(InBoxs[index].confidence.toFixed(4), srcRect.x, srcRect.y - 5);
      }
      ctx.stroke();
      resolve(canvas.toDataURL('image/jpeg', 1));
    };
