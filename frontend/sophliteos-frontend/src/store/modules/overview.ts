...
export const useDeviceInfo = defineStore({
  ...
  getters: {
    needCoreBoard: (state) => state.hasCoreBoard,
  ...
  actions: {
    async getDeviceInfo() {
      ...
      if (result) {
        ...
        // 控制板数据
        this.cpu.unshift({ name: deviceSn, value: cpu.usage.toFixed(1) });
        this.memory.unshift({ name: deviceSn, value: memory.usage.toFixed(1) });
        this.tpu.unshift(null);
        ...
        if (!isSingleBoard && !isPcie) {
          ...
          // 显示 核心板DOKER容器 菜单
          const coreContainMenu = Maintenance?.children?.find(
            (child) => child.name === 'coreContain',
          );
          coreContainMenu && (coreContainMenu.meta.hideMenu = false);
          this.hasCoreBoard = true;
