...
func VersionInit(versionFile string) dto.BuildInfo {
	// 读取文件内容
	fileContent, err := os.ReadFile("/var/lib/sophliteos/release_version.txt")
	if err != nil {
		logger.Error("无法读取文件:", err)
		return dto.BuildInfo{}
	}
...
func parseBuildInfo(content string) dto.BuildInfo {
	...
	for _, module := range modules {
		...
		if moduleName != "" && commit != "" {
			buildInfo.Modules = append(buildInfo.Modules, dto.Module{Name: moduleName, Commit: commit})
		}
	}
