...
func (f *FileService) Wget(w request.FileWget) error {
	command := "wget" + w.Url + "-O" + w.Name
	_, err := cmd.ExecWithDir(command, w.Path)
	if err != nil {
		return err
	}
	return err
	...
	if m.Type == "copy" {
		for _, src := range m.OldPaths {
			if err := fo.Copy(src, m.NewPath); err != nil {
				...
				logger.Error("copy file [%s] to [%s] failed, err: %s", src, m.NewPath, err.Error())
			}
		}
	}
