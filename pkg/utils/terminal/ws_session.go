...
func (sws *LogicSshWsSession) receiveWsMsg(exitCh chan bool) {
	...
	for {
		...
		default:
			...
			// logger.Info("data:%s", string(wsData))
			switch msgObj.Type {
			case WsMsgResize:
			...
			case WsMsgCmd:
				switch msgObj.Data {
				case "G1syOzJS", "G1s+MDsyNzY7MGM=", "G10xMDtyZ2I6ZWNlYy9lY2VjL2VjZWMbXA==", "G10xMTtyZ2I6MDAwMC8wMDAwLzAwMDAbXA==", "G1s/MTI7MiR5", "G1tJ", "G1szOzFS":
					continue
				}
	...
	for {
		...
		case <-tick.C:
			...
			if len(bs) > 0 {
				...
				// if err != nil {
				// 	logger.Error("ssh sending combo output to webSocket failed, err: %v", err)
				// }
				// _, err = sws.logBuff.Write(bs)
				// if err != nil {
				// 	logger.Error("combo output to log buffer failed, err: %v", err)
				// }
				...
				if err != nil {
					logger.Error("encoding combo output to json failed, err: %v", err)
					continue
				}
				err = wsConn.WriteMessage(websocket.TextMessage, wsData)
				if err != nil {
